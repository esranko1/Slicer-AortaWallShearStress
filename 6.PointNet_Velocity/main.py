...
PyTorch, with additions: the inlet velocity waveform is encoded and fused into the
global shape feature (so predictions depend on both geometry and flow condition, not
shape alone), TAWSS is predicted as an auxiliary multi-task target, and the output is
constrained to be non-negative (WSS is a magnitude).
...
New: the velocity encoder and its fusion into the global feature; a second output head
predicting TAWSS alongside SWSS (multi-task learning, sharing the same backbone);
Softplus instead of SiLU on the output heads to guarantee non-negative predictions;
mid-fold checkpointing so an interruption mid-training doesn't lose partial progress.
...
with arrays:
    ...
    b -> (100, 4096, 1)  : SWSS (primary target)
    c -> (100,)          : patient identifiers
    d -> (100, 4096, 1)  : TAWSS (auxiliary multi-task target)
"""
...
class AortaPointNetVelocity(nn.Module):
    def __init__(self, velocity_dim=VELOCITY_DIM):
        ...
        # Two heads share the backbone above. SWSS is the primary target; TAWSS is an
        # auxiliary target — multi-task learning gives the shared features a second,
        # correlated supervisory signal, which typically produces more robust
        # representations than training on SWSS alone.
        self.swss_head = nn.Conv1d(32, 1, kernel_size=1)
        self.tawss_head = nn.Conv1d(32, 1, kernel_size=1)
        # Softplus instead of SiLU: strictly positive output everywhere (WSS is a
        # magnitude and can never be negative), whereas SiLU dips slightly negative
        # for small negative inputs.
        self.pred_act = nn.Softplus()
    ...
    def forward(self, points, velocity):
        ...
        s = self.seg_conv64(s)
        s = self.seg_conv32(s)
        swss_out = self.pred_act(self.swss_head(s))    # (batch, 1, num_points)
        tawss_out = self.pred_act(self.tawss_head(s))  # (batch, 1, num_points)
        return swss_out.squeeze(1), tawss_out.squeeze(1), matrix1, matrix2  # (batch, num_points) x2
...
def compute_single_loss(pred, target, data_range=1.0):
...
def compute_loss(swss_pred, swss_target, tawss_pred, tawss_target, matrix1, matrix2, data_range=1.0):
    """Combined multi-task loss. Returns (total_loss, swss_loss) — swss_loss alone is
    what fold checkpointing/early-stopping should watch, since SWSS is the actual
    target we report on; TAWSS only exists to shape the shared backbone features."""
    swss_loss = compute_single_loss(swss_pred, swss_target, data_range=data_range)
    tawss_loss = compute_single_loss(tawss_pred, tawss_target, data_range=data_range)
    reg_loss = orthogonal_regularization(matrix1) + orthogonal_regularization(matrix2)
    total_loss = swss_loss + AUX_LOSS_WEIGHT * tawss_loss + reg_loss
    return total_loss, swss_loss
...
def train_one_fold(X_train, Y_train, T_train, Z_train, X_test, Y_test, T_test, Z_test, device, fold_idx):
    ...
    for epoch in range(start_epoch, MAX_EPOCHS):
        ...
        for start in range(0, n_train, BATCH_SIZE):
            ...
            optimizer.zero_grad()
            swss_pred, tawss_pred, m1, m2 = model(X_train_t[idx], Z_train_t[idx])
            loss, _ = compute_loss(swss_pred, Y_train_t[idx], tawss_pred, T_train_t[idx], m1, m2)
            loss.backward()
        ...
        model.eval()
        with torch.no_grad():
            val_swss_pred, val_tawss_pred, vm1, vm2 = model(X_test_t, Z_test_t)
            val_loss, val_swss_loss = compute_loss(val_swss_pred, Y_test_t, val_tawss_pred, T_test_t, vm1, vm2)
            val_loss = val_loss.item()
            val_swss_loss = val_swss_loss.item()

        # Checkpoint selection tracks SWSS-only loss, not the combined multi-task loss:
        # TAWSS is only there to help the shared backbone learn, and a fold should never
        # be judged (or have its "best" checkpoint chosen) by how well it happens to
        # predict the auxiliary target.
        if val_swss_loss < best_val_loss:
            best_val_loss = val_swss_loss
        ...
        if epoch % 50 == 0 or epochs_without_train_improvement >= EARLY_STOP_PATIENCE:
            logging.info(
                f"  Fold {fold_idx} epoch {epoch}: train_loss={epoch_train_loss:.4f} "
                f"val_loss={val_loss:.4f} (swss={val_swss_loss:.4f})"
            )
    ...
    model.eval()
    with torch.no_grad():
        final_swss_pred, final_tawss_pred, _, _ = model(X_test_t, Z_test_t)
    ...
    return final_swss_pred.cpu().numpy(), final_tawss_pred.cpu().numpy(), best_val_loss, best_state
...
def main():
    ...
    if 'd' not in data:
        raise RuntimeError(
            f"{DATA_PATH} has no 'd' (TAWSS) array — re-run "
            "1.Data_preprocessing/1.3.convert_mat_to_npz.py to regenerate it "
            "before running the multi-task model."
        )
    a, b, d_arr = data['a'], data['b'], data['d']
    X = a[:, :, :3].astype(np.float32)          # (100, 4096, 3)
    Z = a[:, 0, 3:].astype(np.float32)           # (100, 50)
    Y = b[:, :, 0].astype(np.float32)            # (100, 4096) SWSS, primary target
    T = d_arr[:, :, 0].astype(np.float32)        # (100, 4096) TAWSS, auxiliary target

    logging.info(f"X: {X.shape}, Z: {Z.shape}, Y: {Y.shape}, T: {T.shape}")
    logging.info(f"Y range: [{Y.min():.4f}, {Y.max():.4f}]")
    logging.info(f"T range: [{T.min():.4f}, {T.max():.4f}]")

    # Scale Y and T to [0, 1] independently for training stability; inverse-transform
    # Y for reporting (T never gets reported on — it's auxiliary only).
    ...
    for fold_idx, (train_ix, test_ix) in enumerate(kf.split(X)):
        ...
        if os.path.exists(results_path):
            ...
            logging.info(f"\n=== Fold {fold_idx} ===")
            pred_scaled, _tawss_pred_scaled, best_val_loss, best_state = train_one_fold(
                X[train_ix], Y_scaled[train_ix], T_scaled[train_ix], Z[train_ix],
                X[test_ix], Y_scaled[test_ix], T_scaled[test_ix], Z[test_ix],
                device, fold_idx
            )
